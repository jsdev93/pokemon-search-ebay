(function () {
  ...
  const getPageTitle = async () => {
    ...
    if (isYahooAuctionPage()) {
      ...
      if (headerH1) {
        // Remove delay for immediate processing
  ...
  if (isEbayItemPage() || isYahooAuctionPage()) {
      ...
      const clickImageGallery = () => {
        ...
        if (galleryButton) {
          try {
            ...
            // Silent error handling to save memory
          }
        } else {
    ...
    class EventHandlers {
      ...
      static debounce(func, wait) {
        // Remove debounce delay for immediate execution
    ...
    class IframeManager {
      static update() {
        ...
        // Don't make requests with empty search terms to avoid CORS redirects
        if (!fullSearch || fullSearch.length < 2) {
          if (
            typeof isYahooAuctionPage === "function" &&
            isYahooAuctionPage()
          ) {
            if (state.searchLinkEl) {
              state.searchLinkEl.href = "#";
              state.searchLinkEl.textContent = "No search terms detected";
            }
            if (PreviewManager._container) {
              PreviewManager._container.innerHTML = `<div style="padding:8px;color:#888">No search terms to preview</div>`;
            }
            return;
          }
          return; // Don't update iframe with empty search
        }

        // Remove debug logging to save memory
      ...
      static request(url) {
        ...
        try {
          chrome.runtime.sendMessage(
            ...
            (res) => {
              if (!res || !res.ok) {
                let errorMsg = "Preview unavailable";
                if (res && res.error) {
                  if (res.error === "empty-search-query") {
                    errorMsg = "No search terms detected";
                  } else if (res.error === "redirected-to-categories") {
                    errorMsg = "Search terms too broad";
                  }
                }
                this._container.innerHTML = `<div style="padding:8px;color:#b00">${errorMsg}.</div>`;
                return;
              }
              if (!res.html) {
                this._container.innerHTML = `<div style="padding:8px;color:#b00">No content received.</div>`;
                return;
              }
    ...
    class IframeScroller {
      static scrollDown() {
        ...
        scroll(360);
        // Remove delay for immediate scrolling
        scroll(360);
      }
    }
    ...
    class VisibilityManager {
      static init() {
        ...
        // Add periodic memory cleanup every 5 minutes to reduce overhead
